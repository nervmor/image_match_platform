...
class pash_interface:
    ...
    def match(self, url_src, url_dst):
        ...
        while (True):
            ...
            url = "http://" + self._engine_srv_ip + "/api/engine/phash_match/"
            code, err, data = common.request(url, req_data)
            if code <> 200:
                logging.getLogger('error').error('request src[%s] dst[%s] fail with code[%d] error[%s]',
                                                 url_src, url_dst, code, err)
                break
            if not data:
                logging.getLogger('error').error('request src[%s] dst[%s] respond data is none',
                                                 url_src, url_dst)
                break
            if len(data) != 1:
                logging.getLogger('error').error('request src[%s] dst[%s] respond data is invalid',
                                                 url_src, url_dst)
                break
            r_s = data[url_src]
            if len(r_s) != 1:
    ...
    def feature_get(self, category, pic_wid, pic_high):
        ...
        code, err, data = common.request(url, req_data)
        if code <> 200:
            logging.getLogger('error').error('request pic_wid[%d] pic_high[%d] fail with code[%d] error[%s]',
